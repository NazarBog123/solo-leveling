...
export async function fetchQuests(): Promise<Quest[]> {
	const { data, error } = await supabase.from('quests').select('*').order('id', { ascending: true })
...
export async function resetAllQuests(): Promise<void> {
	const { error } = await supabase.from('quests').update({ completed: false }).neq('id', 0) // target all rows
...
export async function getLastReset(): Promise<Reset | null> {
	const { data, error } = await supabase.from('reset').select('*').order('created_at', { ascending: false }).limit(1).maybeSingle()
...
export async function runDailyResetIfNeeded(): Promise<void> {
	...
	// ── Insert reset record ────────────────────────────────────────────────────
	const { error: resetError } = await supabase.from('reset').insert({ date: today })
